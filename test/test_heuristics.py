...
class HeuristicTestMixin(Base):
    ...
    def test_convergence_sphere(self):
        ...
        result, score = self.optimizer_func(
            functions.sphere, self.bounds_sphere, 
            n_iter=100, n_pop=20, seed=self.seed, verbose=False
        )
        desired = np.zeros(2)
    ...
    def test_convergence_ackley(self):
        ...
        result, score = self.optimizer_func(
            functions.ackley, self.bounds_ackley, 
            n_iter=300, n_pop=100, seed=self.seed, verbose=False
        )
    ...
    def test_performance_vs_random_search(self):
        ...
        if opt_name == 'heuristic' and isinstance(self.optimizer_func, functools.partial):
            opt_name = self.optimizer_func.func.__name__

        self.assertLess(target_score, rs_score, 
            f"Heuristic {opt_name} did not beat Random Search!")

    def test_bounds_respected(self):
        """Ensure results are within bounds"""
        tight_bounds = np.asarray([(0.5, 1.0), (0.5, 1.0)])
        result, _ = self.optimizer_func(
            functions.sphere, tight_bounds, 
            n_iter=20, n_pop=10, seed=self.seed, verbose=False
        )
    ...
    def test_history_debug(self):
        ...
        _, _, debug_info = self.optimizer_func(
            functions.sphere, self.bounds_sphere, 
            n_iter=10, n_pop=10, debug=True, verbose=False
        )
    ...
    def test_callback_early_stopping(self):
        ...
        self.optimizer_func(
            functions.sphere, self.bounds_sphere, 
            n_iter=n_iter, n_pop=20, callback=c.callback, verbose=False
        )
    ...
    def test_callback_patience(self):
        ...
        self.optimizer_func(
            functions.sphere, self.bounds_sphere, 
            n_iter=n_iter_max, n_pop=20, callback=c.callback, verbose=False
        )
        
        self.assertLess(c.epoch, n_iter_max - 1, 
...
class TestHC(HeuristicTestMixin, unittest.TestCase):
    ...
    def test_convergence_ackley(self):
        self.skipTest("Skipping: Hill Climbing (Local Search) naturally fails on Ackley (Multimodal)")

    def test_performance_vs_random_search(self):
        self.skipTest("Skipping: Hill Climbing exploits locally; Random Search explores globally. On Ackley, RS wins.")
...
class TestSA(HeuristicTestMixin, unittest.TestCase):
    ...
    """
    def setUp(self):
        self.optimizer_func = functools.partial(sa.simulated_annealing, step_size=0.1, temp=10.0)
        super().setUp()
    def test_convergence_ackley(self):
        self.skipTest("Skipping: SA (Trajectory Method) gets trapped in Ackley local minima")
...
class TestFA(HeuristicTestMixin, unittest.TestCase):
    def setUp(self):
        self.optimizer_func = functools.partial(fa.firefly_algorithm, beta0=0.005, gamma=0.01, alpha=0.5, alpha_decay=0.98)
        super().setUp()
...
class TestRS(unittest.TestCase):
    ...
    def test_basic_execution(self):
        res, score = rs.random_search(
            ...
            n_iter=10, n_pop=10, seed=self.seed, verbose=False
        )
